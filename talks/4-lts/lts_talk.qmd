...
title-slide-attributes:
  ...
  data-background-size: "35% auto"
  data-background-position: 7.5%
---
...
## The Solution: The LTS Extension

[lts](https://github.com/wjhopper/lts){target="_blank"} is a Quarto extension providing a custom document format and project type

![](img/workflow.png){fig-align="center"}

::: {.fragment}
It allows you to produce all 3 documents from a single `.qmd` file
:::
...
::: {.fragment}
Then manually add the pre-render script to the project's `_quarto.yml` configuration file, e.g.,
...
```{=html}
<iframe src="demo/demo.html" height="600" width="850"></iframe>
```
...
## The Solutions: `lab1_solutions.html`

```{=html}
<iframe src="demo/demo_solutions.html" height="600" width="850"></iframe>
```
...
Let's upload these to our course's Moodle using the [`moodleR` package](https://github.com/wjhopper/moodleR){target="_blank"}!
...
::: {.fragment}
Built on top of the [`chromote`](https://rstudio.github.io/chromote/reference/index.html){target="_blank"} and [`httr`](https://httr.r-lib.org/index.html){target="_blank"} packages
:::
...
In [SDS 100](https://smithcollege-sds.github.io/sds100/){target="_blank"}, we have a [phased group project](https://smithcollege-sds.github.io/sds100/final_project.html#gallery){target="_blank"} where students collaborate on an exploratory data analysis
...
schedule <- split(groups, groups$group_name) |>
  lapply(\(x) {
    sds100::create_rotation(
      group_members = x$name,
      rotations = paste("Phase", 1:4)
    ) |>
...
::: {.fragment}
`{ifingroup 63501}` is a Moodle [FilterCodes](https://marketplace.moodle.com/plugins/filter_filtercodes){target="_blank"} that restricts visibility to members of a specific group
:::
...
- Check out [`r fontawesome::fa("github")` moodleR](https://github.com/wjhopper/moodleR){target="_blank"}
- Check out [`r fontawesome::fa("github")` lts](https://github.com/wjhopper/lts){target="_blank"}
- Get in touch [`r fontawesome::fa("envelope")` whopper@smith.edu](mailto:whopper@smith.edu)
